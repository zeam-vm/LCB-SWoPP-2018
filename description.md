...
Node.jsでは，コールバックを用いてI/Oを非同期的に扱ってノンプリエンプティブなマルチタスク処理にする機構，Nodeプログラミングモデルが備わっている\cite{Node}．これにより，ウェブサーバーのメモリ使用量を格段に減らすことができ，同時セッション最大数やレイテンシが改善される\cite{NodeBenchmarks}．
...
我々は Nodeプログラミングモデル\cite{Node}に着想を得て， \Cpp で実装した，コールバック関数を用いてマルチタスク処理にするライブラリであるZackernelを開発した\footnote{Zackernel: an Engine for IoT, available at \url{https://github.com/zackernel/zackernel}}．同様のものに libevent\cite{libevent}があるが，Zackernel は \Cpp 11 で導入された匿名関数を用いることで可読性を改善している．
...
Zackernelでのアプリケーションプログラム例を\figref{fig:Zackernel_code}に示す．
...
Zackernelのコード行数は約900行であり，すべて \Cpp で記述されておりアセンブリ言語を一切含まない．プロセスやスレッドでマルチタスクを実現した場合には必ずアセンブリ言語記述を含み機種依存が生じてしまうが，Zackernelではこの問題は生じない．ZackernelをmacOSで静的ライブラリとして構成した場合のサイズは，合計約134KBである．

# 軽量コールバックスレッド

我々は，Nodeプログラミングモデル\cite{Node}に着想を得て，Elixir で実装した，コールバック関数を用いてマルチタスク処理にするライブラリである軽量コールバックスレッド\footnote{ZeamCallback: 
ZeamCallback, available at \url{https://github.com/zeam-vm/zeam_callback}}を開発した．現状では Elixir で記述されている．
...
軽量コールバックスレッドの基本機能は正味約150行で構成される．Elixirのみで記述されており，他言語は用いていない．コンパイルした時のオブジェクトファイルである BEAM コードのサイズは合計約12KBである．
...
本研究ではNodeプログラミングモデルと同様の機構でマルチタスク処理をするを \Cpp と Elixir で実装し，それぞれ Zackernel，軽量コールバックスレッドと呼称した．
